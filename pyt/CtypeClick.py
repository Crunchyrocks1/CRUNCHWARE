...
# pyDirectInput buy changed names incase AC checked for lib named "PyDirectInput"
...
def failSafeCheck():
    if FAILSAFE and tuple(position()) in FAILSAFE_POINTS:
        raise FailSafeException(
            "m_CtypeClick fail-safe triggered from mouse moving to a corner of the screen. To disable this " \
            "fail-safe, set m_CtypeClick.FAILSAFE to False. DISABLING FAIL-SAFE IS NOT RECOMMENDED."
        )
...
def _genericm_CtypeClickChecks(wrappedFunction):
